...
# --- Input Parameters --- all comes from the burin with multitrait scenario, but here we only select one trait
burin_dir <- "../../Multivariate/Burnin_Gen_MVN/burnin_states"
results_dir <- paste0("results_", SCENARIO)
output_csv_3f <- file.path(results_dir, paste0("UNV_MtR_3F_Results_", REP, ".csv"))

# ---- Create Results Directory ----
if (!dir.exists(results_dir)) dir.create(results_dir)

# ---- Initialize CSV File ----
if (file.exists(output_csv_3f)) file.remove(output_csv_3f) # Remove existing file to avoid appending
...
correlation_file <- file.path(correlation_dir, paste0("Correlation_true_unv", REP, ".csv"))
...
write.table(t(header), file = output_csv_3f, sep = ",", col.names = FALSE, row.names = FALSE, quote = FALSE)
...
for (year in (nBurnin+1):(nBurnin+nfuture)) {
  ...
  unique_position <- match(id_unique, ids_all)
  M_train <- M_all[unique_position, ] #filter to have only once each genotype, maybe thats what filter_zero_variance does
  rm(M_all)
  ...
  model <- lmeb(Yield~ YearEff + #fixed effect is the year effect
                  (1|GID),
                relmat = list(GID=GRM),
                verbose = 0L, trace = 0L,
                data = y_train)
  ...
  genicSD_C <- sqrt(as.numeric(genicVarG(ParentalPool)[1]))
  ...
  output$genic_SDlost[year] <- genicSD_lost_CPw
  
  
    
  family_vect <- names(out$FamilyValues$mean)
  
  Parents@ebv <- setGEAVs(
    Parents, marker_eff
  )
  ...
  print(output$meanG_yield[year])
  print(output$meanG_yield[year-1])
}

# Append results to CSV file
write.table(output[output$rep == REP & output$year > nBurnin, ],
            file = output_csv_3f, sep = ",", col.names = FALSE, row.names = FALSE, append = TRUE)

cat("MtR phase completed and results saved in '", output_csv_3f, "'.\n")
