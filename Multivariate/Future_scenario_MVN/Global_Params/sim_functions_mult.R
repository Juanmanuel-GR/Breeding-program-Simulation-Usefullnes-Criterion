...
setGEAVs <- function(geno_obj, addEffs_list, trait_name=NULL) {
  #  pull new individuals' SNP matrix (rows = individuals, cols = SNPs)
  ...
  # filter to the SNPs present in the reference marker set
  ...
  # extract the effect vector for the chosen trait, default all the traits
  if (!is.null(trait_name)) {
    ...
    #  compute GEBVs as matrix multiplication
...
getVR_GRM <- function(Markers) {
  ...
  #  estimate allele frequencies p_j = mean genotype / 2
  pvec <- colMeans(M) / 2
  
  #  center each column: subtract 2 * p_j
  #    (scale=FALSE so only centering)
  Z <- scale(M, center = TRUE, scale = FALSE)
  
  #  compute denominator: 2 * sum[p_j * (1 - p_j)]
...
select_index_within <- function(Population, coefficients, nFamMax, pattern = "phenotype") {
  ...
  #  Extraer la matriz de datos según el patrón ("phenotype" o "genotype")
  ...
  # . Calcular el Índice (Suma ponderada)
  ...
  #  Crear población temporal para la selección
...
geno_selection_index <- function(population ,economicWeight, number_selected) {
  ...
  #  Calcular el Índice usando sweep (multiplica cada fila por el peso)
